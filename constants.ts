...
5. 각 프레임에 어울리는 시각 자료(이미지 또는 비디오)를 묘사하는 프롬프트(visualPrompt)를 영어로 작성하세요. 
...
6. 시각 유형(visualType) 결정 규칙:
   - **기본값은 'IMAGE'입니다.**
   - 'VIDEO'는 움직임 표현이 교육적 이해를 돕는 데 필수적인 경우(예: 기계 작동, 물리적 이동 등)에만 제한적으로 선택하세요.
7. **중요:** '한국수력원자력'의 영문 약어는 반드시 **'KHNP'**로 표기하세요. (KNHP, KHN 등 오표기 금지)
8. JSON 형식으로만 응답하세요.
`;